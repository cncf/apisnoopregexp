...
while true
do
  res=`sudo -u postgres psql hh -tAc 'select count(distinct auditid) filter (where opid is not null) as found, count(distinct auditid) filter (where opid is null) as not_found from audit_events'`
